...
| 2026-09-19 | **Zenodo: the record was wrong, and the public DOI still carries retracted claims** | Preparing v3.11.0: what does Zenodo actually hold? | **Version 1.5.5 (2026-04-05), not v3.10.0** as the 2026-09-16 wrap-up had recorded. Checked on zenodo.org and its API: version DOI `10.5281/zenodo.19429983`, concept DOI `10.5281/zenodo.19424985`. Its description still claims validation against Muskingum Watershed chemistry and names Ganau Pond, both retracted. **Cause:** Zenodo's GitHub integration archives GitHub **Releases**. The only Releases are 1.5.4 and 1.5.5; every v2.x and v3.x version was a tag only. **Also found:** the v3.10.0 tag points at the 2026-09-09 CMD3 commit, so the 2026-09-13 record corrections were never tagged. The retraction notice said "≤ 1.5.4" while 1.5.5 carries the same claims. **Done:** `CITATION.cff` → 3.11.0 with the concept DOI and a blind-search sentence; README badge and BibTeX; a CHANGELOG `[3.11.0]` entry; `HOW_TO_TEST.md` §E rewritten (a Release, not a tag; replace the pre-filled description if uploading by hand; flag 1.5.5 as superseded); the 2026-09-16 summary corrected in place, not rewritten. **Not done, the author's:** tag, Release, Zenodo check. **Why it matters now:** GSA is 11 Oct, and the DOI is what reviewers will click | — |
| 2026-09-19 | **GEE tool audit: the retracted water module was ON by default** | The author asked whether the Earth Engine code had been updated. What does it assert to someone who opens it? | **It shipped the retracted capability switched on.** The tool opened with a "Water Quality Classification" layer over every water body reading **clean / moderate / severe contamination**, a ticked enable box, help text promising it "detects sulfate/iron contamination in water bodies", a legend pointing to it, and a header claiming the method extends "to contaminated water bodies quality assessment". Those indices ranked a chemically **clean control lake highest** (W1), and the water column is a measured null (B1: no feature's CI excluded zero, iron n=17, sulfate n=23; turbidity detectable at n=50). **Also dead code:** the validation printout counted land classes 20/21, removed in v2.4.0, so it printed 0.00% while implying a water measurement. **Fixed in tool v3.1.1:** off and hidden by default, everything relabelled RETRACTED, a Console warning when enabled, and the land classification untouched. **A presentation defect: no number moves.** Every other externally-facing document was corrected on 2026-09-13; the tool's own defaults were missed then, which is why the artefact a GSA audience is most likely to open was the last one still asserting it | tool v3.1.1 |
| 2026-09-19 | **SpectraLab AMD module: first full UI run** (gate item closed) | Does the module work when driven through the interface, not a test harness? | **Yes, and it found a defect.** A two-district AOI (Silverton + Ouray, 1,950 km²) ran end to end in **1 min 15 s**: 39 stations from a 134-scene composite, `amd_report.md` and both CSVs written, the report carrying the EXPLORATORY banner, the preset's validation history and the full claim boundaries **including the blind-search MAY-NOT line**. Status was *completed with warnings*. **The defect:** the score raster was lost to "User memory limit exceeded". `export_geotiff` coarsened for the 32 MB **size** cap and `export_png` halved on a **memory** error, but `export_geotiff` had no memory path — and EE's memory limit is about computing the composite, not the file size. Statistics were unaffected: they finish per station buffer before the raster is requested. **Fixed:** double the scale and retry, up to 16x, reporting the scale used, since a coarsened raster is display only. **Verified live on the AOI that failed:** 30 → 60 → 120 m, 1.3 MB, 24 s. 212 tests. **The run's own statistics are exploratory** (interactive AOI and grouping) and enter no claim | v3.11.1 |
| 2026-09-19 | **SpectraLab 0.40.0 released and installed** | Can the AMD module ship, and what does shipping it require? | **Merged, built as a wheel, installed; the exe is blocked on disk.** The packaging question was the real blocker: `amdtool` is **not on PyPI**, so a wheel or exe could ship a module that can never run. Now an **`amd` extra** pins `amdtool @ git+…/AMD-Detection-Tool@v3.11.1` (folded into `desktop`, with `allow-direct-references`), and **`amdtool` is in `build_exe.py`'s `_REQUIRED_AT_RUNTIME`** — without that, a build from an environment lacking it **succeeds silently**, the same trap that produced a nicegui-less exe in 0.25.0. Merged fast-forward to `main`, tagged `v0.40.0`; wheel `94fc7791…` in `releases/`; installed as a uv tool, where the app now lists modules `amd`, `stepwise`, `vpca` and imports `amdtool` from the pinned tag rather than the editable checkout. **The exe was NOT built: drive D: hit 0 bytes free** and PyInstaller died with `No space left on device`; nothing of the author's was deleted, and the failure is recorded in `RELEASES.md` rather than hidden. Science invariants untouched; 509 passed, 4 skipped | SpectraLab v0.40.0 |
| 2026-09-20 | **SpectraLab 0.40.0 exe built and verified** | Does the packaging fix actually produce a working exe, not just a successful build? | **Yes.** The author freed disk space (D: had hit 0 bytes free); the exe then built (159 MB, SHA-256 `b14327dff4…a6b91`). Run once rather than trusted on faith: it serves on `localhost:8080`, its own UI reports **v0.40.0**, and the module picker lists **"AMD severity report"** — confirming the `amd` extra and `_REQUIRED_AT_RUNTIME` addition from 2026-09-19 genuinely bundle `amdtool` into a frozen build. That distinction mattered: before the fix, a build missing `amdtool` succeeded silently and shipped a dead module | SpectraLab v0.40.0 exe |
| 2026-09-22 | **GSA Connects 2026 talk — framing, figures and deck** | Is there enough for a defensible 12-minute talk, is there too much, and how should it be framed? | **Enough several times over; the risk was overload, not emptiness.** Decided with the author: hook on cryptic pollution (the published title), body = the measured envelope of the satellite arm, one bridge slide for coal, **self-correction front and centre**, and the pre-registered campaign as the close. **The liability addressed head-on:** the abstract promised δ³⁴S, XRD/XRF/ICP-OES, EMIT×ECOSTRESS fusion, ML automation and magnetism — none of which has measured data on the author's own material (the FORC file on disk is a class specimen, not a lake sample), so slide 3 states the scope change and its scientific reason rather than hoping nobody read the abstract. **Built:** eight figures from committed data/reports, a 14-slide deck with per-slide timings (1,696 spoken words ≈ 11.7 min against a 12-minute slot, with slide 8 as the designed cut), a twelve-question Q&A sheet, and a PDF backup. **Traceability is enforced in code:** the figure script re-reads each source report and aborts if a literal has drifted, and the dose-response figure recomputes ρ from raw extractions and aborts unless it reproduces +0.568 at n=75 with +0.64/+0.68/+0.64/+0.004. **Go/no-go:** recommended GO — registration is non-refundable, the cancellation deadline passed on 10 September, and the $300 GSS travel award was granted 21 September, so the marginal cost is flight + lodging + food − $300 | — |
| 2026-09-20 | **Snow-masked sensitivity — the last registered analysis** | Was the blind-search null an artefact of unmasked snow? | **No, and neither verdict moves.** On a composite that also masks SCL 11, **H-BS1 falls to 3 of 32** (recall 0.094 [0.032, 0.242], p = 0.2139, was 4 of 32 at p = 0.074) and **H-BS2 to 12 of 50** (recall 0.240 [0.143, 0.374], was 14). Recall drops slightly in both — the only direction in which this could have overturned anything — so **masking snow does not rescue discovery**. H-BS2's NOT BETTER THAN BARE GROUND stands untouched: the sensitivity reports recall only and does not recompute the McNemar comparison. n is unchanged at 32 and 50; the sites are identical and only the composite differs. **Reproducibility:** rerunning `--analyse` reproduced every other line of the 2026-09-15 report **byte-for-byte**, five days on. **Operationally:** the first pass extracted six of seven districts and **Ouray timed out**; under the rule committed before any result (`724acc8`) that is transient, so it was re-extracted rather than reported FAILED, and then succeeded. Registered as a sensitivity in §9 and reported as one: **never a verdict** | — |
