...
# Addendum, 2026-09-20 — the snow-masked sensitivity, now run

The one registered analysis (§9) left unrun on 2026-09-15 has been run. It is a
**sensitivity, never a verdict**, and it changes neither verdict.

**What it is:** the same registered sites and the same 5% flagged-area budget,
scored on a Sentinel-2 composite that also masks SCL class 11 (snow). The
primary composite deliberately does not mask snow; the question was whether
bright snow was inflating or suppressing what the index flags.

| hypothesis | primary composite | snow-masked | change |
|---|---|---|---|
| **H-BS1** (districts never used to choose the index) | 4 of 32, recall 0.125 [0.050, 0.281], p = 0.074 | **3 of 32**, recall **0.094** [0.032, 0.242], p = 0.2139 | one site fewer |
| **H-BS2** (districts that chose the index) | 14 of 50, recall 0.280, p = 1.0e-7 | **12 of 50**, recall **0.240** [0.143, 0.374], p < 0.0001 | two sites fewer |

**Reading it:**
- **Masking snow does not rescue discovery.** Recall falls slightly in both
  hypotheses, so the null on H-BS1 is not an artefact of unmasked snow — the
  only direction in which this sensitivity could have overturned anything.
- **H-BS1 stays NO SIGNAL DETECTED**, further from its bar than before.
- **H-BS2's verdict is untouched.** Its NOT BETTER THAN BARE GROUND verdict came
  from the McNemar comparison against the bare-ground baseline, which this
  sensitivity does not recompute; the snow-masked block reports recall only.
- **n is unchanged** at 32 and 50 sites: the sites are identical, and only the
  composite differs.

**Reproducibility, five days on:** rerunning `--analyse` with the snow-masked
extractions present reproduced **every other line of the 2026-09-15 report
byte-for-byte** — every primary number, every sensitivity, the frame counts and
T-86. The only difference is the snow block replacing its "not extracted" line.
→ [`report_blind_search_snowmask_2026-09-20.txt`](report_blind_search_snowmask_2026-09-20.txt)

**Operational note, under the rule committed before any result (`724acc8`):**
the first snow-masked pass extracted six of seven districts; **Ouray failed with
`EEException: Computation timed out`**. That is a transient Earth Engine compute
error, not a property of the data, so Ouray was re-extracted rather than
reported FAILED. It then succeeded with the same code (120 scenes, 2,000
landscape points, 73 stations) after Earth Engine's batch halving. All seven
districts are present in the numbers above.

## Reproduce this addendum

```
python python/blind_search.py --extract --snow-masked
python python/blind_search.py --analyse --out validation/report_blind_search_snowmask_2026-09-20.txt
```
