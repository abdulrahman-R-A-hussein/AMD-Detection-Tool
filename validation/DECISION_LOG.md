...
| 2026-09-14 | **Audit** while scoping the next test | Is anything the blind-search test and the SpectraLab module would build on defective? | **Six defects, all verified.** Arm A σ computed per catchment; Arm A's 31 rows = **28** distinct polygons, 3 leaking across LORO; Creede/Lake City **geology mislabelled** (USGS I-2799, I-962) and OPEN #1's recommendation to test on the generating data **withdrawn**; `convolve_splib07` crashes on NumPy 2.x; CMD2 bars don't transfer to PA; a comment claimed an extraction that never happened. **No retraction rescued** | — |
